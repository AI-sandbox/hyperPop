...
enc_dec_dict = {"fc_wrapped_encoder": fc_wrapped_encoder, "fc_wrapped_decoder": fc_wrapped_decoder, "fc_geodesic_decoder": fc_geodesic_decoder, "fc_wrapped_decoder_rawvals": fc_wrapped_decoder_rawvals} 
...
def main():
    ...
    if args["dataset"] == "ancestry":
        ...
        pca_model = PCA()
        if len(dataset_train) > 2000:
            print("Large dataset")
            np.random.seed(0)
            pca_split = np.random.choice(len(dataset_train), 2000, replace=False)
            pca_model.fit(dataset_train.snps[pca_split])
            dataset_train.snps = pca_model.transform(dataset_train.snps)
        else:
            dataset_train.snps = pca_model.fit_transform(dataset_train.snps)
        print(dataset_train.snps.shape)
        dataset_valid.snps = pca_model.transform(dataset_valid.snps)
        joblib.dump(pca_model, args["output_dir"] + "pca_model.joblib")
